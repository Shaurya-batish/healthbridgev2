...
def list_escalations(facility_id: uuid.UUID, db: Session = Depends(get_db)) -> list[EscalationResponse]:
...
def acknowledge_escalation(escalation_id: uuid.UUID, db: Session = Depends(get_db)) -> EscalationResponse:
