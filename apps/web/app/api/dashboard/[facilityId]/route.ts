...
export async function GET(_req: Request, { params }: { params: { facilityId: string } }) {
  try {
    const tiles = await coreRequest(`/dashboard/${encodeURIComponent(params.facilityId)}`, {
      headers: authHeader(getSessionToken()),
    });
    return NextResponse.json(tiles);
  } catch (err) {
    if (err instanceof UpstreamError) {
      return NextResponse.json(err.body ?? { detail: "dashboard_unavailable" }, { status: err.status });
    }
